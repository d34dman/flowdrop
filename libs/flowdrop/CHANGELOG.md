...
- **New chat types** in `@flowdrop/flowdrop/chat`: `ChatToolDefinition`, `ChatToolCall`, `ChatToolResult`, `ChatToolResultsRequest`, `ChatTurnRequest`, `ChatTurnResponse`. New strings under `defaultMessages.chat.tools` (`reading`, `awaitingApproval`, `applied`, `rejected`, `failed`, `rounds`, `legacyFallback`, `aborted`). The Settings panel gained "AI Assistant Mode". The MSW mock backend scripts one round of reads for a tool-calling turn so the dev pages exercise the mode.
